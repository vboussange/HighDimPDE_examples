...
function nonlocal_comp(;d,tspan,dt,batch_size,train_steps,σ_sampling,K)
        ...
        a(u) = u - u^3
        f(y, z, v_y, v_z, ∇v_y ,∇v_z, t) =  max.(0f0, v_y) .*(1 - max.(0f0, v_z) * (2 * π )^(d/2) * σ_sampling^d)
        μ_f(X,p,t) = 0.0f0 # advection coefficients
        σ_f(X,p,t) = sqrt(1f-1) # diffusion coefficients
        mc_sample(x) = NormalSampling(σ_sampling,true) # uniform distrib in u_domain
