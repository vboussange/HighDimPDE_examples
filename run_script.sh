...
# examples=("nonlocal_comp" "nonlocal_sinegordon" "rep_mut")
for i in {0..4}; do
    ...
    # ($HOME/utils/julia-1.7.2/bin/julia --project=. --threads 1 "$pathsim.jl" $SEED $c &> "stdout/${namesim}_$c.out") &
     ($HOME/utils/julia-1.7.2/bin/julia --project=. --threads=20 run_example_files_arg_parse.jl $i ${examples["$i"]} &> stdout/${examples["$i"]}.stdout) &
done
wait
